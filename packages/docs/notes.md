...
```sh
docker run -e XDG_RUNTIME_DIR=/tmp -e WAYLAND_DISPLAY=$WAYLAND_DISPLAY -v $XDG_RUNTIME_DIR/$WAYLAND_DISPLAY:/tmp/$WAYLAND_DISPLAY --name playwright --net host --rm --init mcr.microsoft.com/playwright:v1.58.2 /bin/sh -c "cd /home/pwuser && npx -y playwright run-server --port 3333 --host 0.0.0.0"
