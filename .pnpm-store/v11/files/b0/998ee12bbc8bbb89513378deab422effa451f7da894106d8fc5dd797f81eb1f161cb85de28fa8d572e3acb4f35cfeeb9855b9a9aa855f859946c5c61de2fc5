...
export default class FetchBodyUtility {
    ...
    static cloneBodyStream(window, requestOrResponse) {
        ...
        if (requestOrResponse.body[PropertySymbol.nodeStream]) {
            ...
            requestOrResponse[PropertySymbol.body] = new ReadableStreamWrapper(() => this.nodeToWebStream(stream1));
            // Returns the clone.
     ...
     * @returns ReadableStream
     */
    static getReadableStream(value) {
        return new ReadableStreamWrapper(() => new ReadableStream({
            start(controller) {
