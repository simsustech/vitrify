...
export default class SelectorItem {
     ...
     * @returns Result.
     */
    match(scope, element, ignoreErrors) {
     ...
     * @returns Result.
     */
    matchPseudo(scope, element, ignoreErrors) {
        ...
        for (const pseudo of this.pseudos) {
            ...
            const selectorMatch = this.matchPseudoItem(scope, element, parentChildren, pseudo, ignoreErrors);
            if (!selectorMatch) {
                return null;
            }
     ...
     * @param [ignoreErrors] Ignores errors.
     */
    matchPseudoItem(scope, element, parentChildren, pseudo, ignoreErrors) {
     ...
     * @returns Result.
     */
    matchChildOfElement(scope, selectorItem, element, ignoreErrors) {
        for (const child of element[PropertySymbol.elementArray]) {
     ...
     */
    getRootElement(scope) {
        return (scope?.[PropertySymbol.ownerDocument]?.documentElement ||
