...
function shouldUpdateDependencyBasedOnConfig(cwd, release, {
  depVersionRange,
  depType
}, {
  minReleaseType,
  ...
  if (usesWorkspaceRange) {
    ...
    switch (depVersionRange) {
      ...
      default:
        {
          if (!validRange__default["default"](depVersionRange)) {
            return path__default["default"].posix.normalize(depVersionRange) === path__default["default"].relative(cwd, release.dir).replace(/\\/g, "/");
          }
          // fallthrough
        }
    }
...
async function getChangelogEntry(cwd, release, releases, changesets, changelogFuncs, changelogOpts, {
    ...
    if (deps) {
      for (let {
        ...
        if (usesWorkspaceRange) {
          ...
          if (workspaceDepVersion === "*" || workspaceDepVersion === "^" || workspaceDepVersion === "~" || validRange__default["default"](workspaceDepVersion) === null) {
            continue;
          }
...
async function applyReleasePlan(releasePlan, packages, config$1 = config.defaultConfig, snapshot, contextDir = __dirname) {
  ...
  if (releasePlan.preState !== undefined && snapshot === undefined) {
    if (releasePlan.preState.mode === "exit") {
      await fs__default["default"].remove(path__default$1["default"].join(cwd, ".changeset", "pre.json"));
    } else {
      await fs__default["default"].writeFile(path__default$1["default"].join(cwd, ".changeset", "pre.json"), JSON.stringify(releasePlan.preState, null, 2) + "\n");
    }
    touchedFiles.push(path__default$1["default"].join(cwd, ".changeset", "pre.json"));
  }
  ...
  for (let release of finalisedRelease) {
    ...
    const pkgJSONPath = path__default$1["default"].resolve(dir, "package.json");
    await updatePackageJson(pkgJSONPath, packageJson);
    touchedFiles.push(pkgJSONPath);
    if (changelog && changelog.length > 0) {
      const changelogPath = path__default$1["default"].resolve(dir, "CHANGELOG.md");
  ...
  if (releasePlan.preState === undefined || releasePlan.preState.mode === "exit") {
    let changesetFolder = path__default$1["default"].resolve(cwd, ".changeset");
    await Promise.all(changesets.map(async changeset => {
      let changesetPath = path__default$1["default"].resolve(changesetFolder, `${changeset.id}.md`);
      let changesetFolderPath = path__default$1["default"].resolve(changesetFolder, changeset.id);
...
async function getNewChangelogEntry(releasesWithPackage, changesets, config, cwd, contextDir) {
  ...
  let changesetPath = path__default$1["default"].join(cwd, ".changeset");
  let changelogPath;
  try {
  ...
  let possibleChangelogFunc = await (function (t) { return Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(t)); }); })(changelogPath);
...
async function prependFile(filePath, data, name, prettierInstance) {
  ...
  if (isVersionHeading) {
    ...
    newChangelog = index === -1 ? fileData + data // treat the whole file as header
